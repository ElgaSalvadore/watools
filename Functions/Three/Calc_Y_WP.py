...
def Season(startdate, enddate, dir_nc_outname, lu_class, croptype, ab = (1.0,0.9)):
    ...
    if not (os.path.exists(nc_outname_start) or os.path.exists(nc_outname_end)):
        ...
        areas_m2 = watools.Functions.Start.Area_converter.Degrees_to_m2(nc_outname_start)
...
def read_csv(csv_fh):
    ...
    reader = np.genfromtxt(csv_fh, dtype = str, skip_header=1, delimiter = ';')
    if len(reader.shape) == 1:
        reader = reader.reshape((1,int(reader.shape[0])))
