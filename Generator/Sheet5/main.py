...
def Calculate(WA_HOME_folder, Basin, P_Product, ET_Product, ETref_Product, DEM_Product, Water_Occurence_Product,
    ...
    Additional_Months_tail = np.max(list(Moving_Window_Per_Class_dict.values()))
    ...
    for year in years:
        ...
        if not "Precipitation" in Variables_NC:
            Data_Path_P_Monthly = Start.Download_Data.Precipitation(Dir_Basin,
                                                                    [Boundaries['Latmin'],Boundaries['Latmax']],
                                                                    [Boundaries['Lonmin'],Boundaries['Lonmax']],
                                                                    Startdate_part, Enddate_part, P_Product)

        if not "Actual_Evapotranspiration" in Variables_NC:
            Data_Path_ET = Start.Download_Data.Evapotranspiration(Dir_Basin,
                                                                  [Boundaries['Latmin'],Boundaries['Latmax']],
                                                                  [Boundaries['Lonmin'],Boundaries['Lonmax']],
                                                                  Startdate_part, Enddate_part, ET_Product)

        if (WaterPIX_filename == "" or Supply_method == "Fraction") \
                and not ("Reference_Evapotranspiration" in Variables_NC):

            Data_Path_ETref = Start.Download_Data.ETreference(Dir_Basin,
                                                              [Boundaries['Latmin'],Boundaries['Latmax']],
                                                              [Boundaries['Lonmin'],Boundaries['Lonmax']],
                                                              Startdate_Moving_Average_String, Enddate_part,
                                                              ETref_Product)

        if Reservoirs_GEE_on_off == 1 and not ("Water_Occurrence" in Variables_NC):
            Data_Path_JRC_occurrence = Start.Download_Data.JRC_occurrence(Dir_Basin,
                                                                          [Boundaries['Latmin'],Boundaries['Latmax']],
                                                                          [Boundaries['Lonmin'],Boundaries['Lonmax']],
        ...
        # WaterPIX input
        Data_Path_DEM_Dir = Start.Download_Data.DEM_Dir(Dir_Basin, [Boundaries['Latmin'],Boundaries['Latmax']],
                                                        [Boundaries['Lonmin'],Boundaries['Lonmax']],
                                                        Resolution, DEM_Product)

        Data_Path_DEM = Start.Download_Data.DEM(Dir_Basin, [Boundaries['Latmin'],Boundaries['Latmax']],
                                                [Boundaries['Lonmin'],Boundaries['Lonmax']],
                                                Resolution, DEM_Product)
        ...
        if not "Precipitation" in Variables_NC:
            ...
            DataCube_Prec = RC.Get3Darray_time_series_monthly(Data_Path_P_Monthly, Startdate_part, Enddate_part,
        ...
        if not "Actual_Evapotranspiration" in Variables_NC:
            ...
            DataCube_ET = RC.Get3Darray_time_series_monthly(Data_Path_ET, Startdate_part, Enddate_part,
                                                            Example_data = Example_dataset)
        ...
        if (WaterPIX_filename == "" or Supply_method == "Fraction") and not \
            ...
            # Get the data of Precipitation and save as nc
            DataCube_ETref = RC.Get3Darray_time_series_monthly(Data_Path_ETref, Startdate_part, Enddate_part,
                                                               Example_data = Example_dataset)
        ...
        if (Supply_method == "Fraction" and not "Surface_Runoff" in Variables_NC):
            ...
            DataCube_Runoff = Five.Fraction_Based.Calc_surface_runoff(Dir_Basin, nc_outname, Startdate_part,
                                                                      Enddate_part, Example_dataset, ETref_Product,
                                                                      P_Product)
        ...
        if (Supply_method == "WaterPIX" and not "Surface_Runoff" in Variables_NC):
            ...
            DataCube_Runoff = Five.Read_WaterPIX.Get_Array(WaterPIX_filename, WaterPIX_Var, Example_dataset,
        ...
        if (Supply_method == "Fraction" and not "Surface_Withdrawal" in Variables_NC):
            DataCube_surface_withdrawal = Five.Fraction_Based.Calc_surface_withdrawal(Dir_Basin, nc_outname,
                                                                                      Startdate_part, Enddate_part,
                                                                                      Example_dataset, ETref_Product,
                                                                                      P_Product)
        ...
        if (Supply_method == "WaterPIX" and not "Surface_Withdrawal" in Variables_NC):
            ...
            DataCube_Supply = Five.Read_WaterPIX.Get_Array(WaterPIX_filename, WaterPIX_Var, Example_dataset, Startdate,
                                                           Enddate)
